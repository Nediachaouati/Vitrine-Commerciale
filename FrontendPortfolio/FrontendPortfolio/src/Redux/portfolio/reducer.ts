...
export default function PortfolioReducer(state = INIT_STATE, action: any): PortfolioState {
    switch (action.type) {
        ...
        //public slug
        case PortfolioActionTypes.LOAD_PORTFOLIO_BY_SLUG:
            ...
            switch (action.payload.actionType) {
                ...
                case PortfolioActionTypes.DELETE_PORTFOLIO:
                    return {
                        ...
                        portfolios: state.portfolios.filter((p) => p.portfolioId !== action.payload.data?.portfolioId),
                        selectedPortfolioId: state.selectedPortfolioId === action.payload.data?.portfolioId ? (state.portfolios[0]?.portfolioId ?? null) : state.selectedPortfolioId,
                    };
                ...
                case PortfolioActionTypes.UPDATE_PORTFOLIO:
                    return {
                        ...
                        portfolios: state.portfolios.map((p) => (p.portfolioId === action.payload.data?.portfolioId ? { ...p, ...action.payload.data } : p)),
                    };
                ...
                case PortfolioActionTypes.CHAT_SEND_MESSAGE:
                    return {
                        ...state,
                        isChatLoading: false,
                        chatMessages: [...state.chatMessages, { role: 'assistant' as const, content: action.payload.data?.message ?? '' }],
                    };

                case PortfolioActionTypes.LOAD_PORTFOLIO_BY_SLUG:
                    return { ...state, loading: false, currentPortfolio: action.payload.data };
