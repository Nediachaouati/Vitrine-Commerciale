...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pf-download-btn:hover { background: var(--ink); transform: translateY(-1px); }
.pf-download-btn:disabled { background: var(--muted); cursor: not-allowed; transform: none; }
.pf-download-btn svg { width: 13px; height: 13px; flex-shrink: 0; }
  ...
  @media print {
  .pf-download-btn, .pf-switcher { display: none !important; }
}
`;
...
const avatarUrl = (first: string, last: string, url?: string) => url || `https://ui-avatars.com/api/?name=${encodeURIComponent(first + '+' + last)}&background=0a0a0f&color=f5f2eb&size=200`;
                ...
                {visibleSkills.map((ps: any) => {
                    ...
                    return (
                        <div key={ps.collabSkillId} className="pf-skill-card">
                            ...
                            {s.yearsUsed > 0 && (
                                <div className="pf-skill-years">
                                    {s.yearsUsed} an{s.yearsUsed > 1 ? 's' : ''}
                                </div>
                            )}
            ...
            <div className="pf-timeline">
                {[...visibleExperiences]
                    ...
                    .map((pe: any) => {
                        ...
                        if (!e) return null;
                        const dateStr = [fmtDate(e.startDate), e.isCurrent ? 'Présent' : fmtDate(e.endDate)].filter(Boolean).join(' — ');
                        return (
                                ...
                                {e.technologies && (
                                    <div className="pf-tl-techs">
                                        {e.technologies.split(',').map((t: string) => (
                                            <span key={t.trim()} className="pf-tech-pill">
                                                {t.trim()}
                                            </span>
                                        ))}
                                    </div>
        ...
        <section className="pf-section" id="education">
            ...
            <div className="pf-timeline">
                {visibleEducations.map((pe: any) => {
                    ...
                    if (!ed) return null;
                    const dateStr = [fmtDate(ed.startDate), ed.isCurrent ? 'Présent' : fmtDate(ed.endDate)].filter(Boolean).join(' — ');
                    return (
                        <div key={pe.educationId} className="pf-tl-item visible">
                            ...
                            {ed.description && (
                                <div className="pf-tl-desc" style={{ marginTop: 8 }}>
                                    {ed.description}
                                </div>
                            )}
                        </div>
...
// ─────────────────────────────────────────────
// Hook PDF — dans MyPortfolioPage.tsx
// ─────────────────────────────────────────────

const usePortfolioDownload = () => {
    const [isDownloading, setIsDownloading] = useState(false);

    const downloadSite = useCallback(async (firstName: string, lastName: string) => {
        setIsDownloading(true);
        try {
            const rootEl = document.querySelector('.pf-root') as HTMLElement;
            if (!rootEl) throw new Error('Élément introuvable');

            // Cloner et nettoyer
            const clone = rootEl.cloneNode(true) as HTMLElement;
            clone.querySelectorAll('.pf-download-btn, .pf-switcher').forEach(el => el.remove());
            ...
            await Promise.allSettled(
                Array.from(images).map(async (img) => {
                    ...
                    } catch { /* garde l'URL originale si fetch échoue */ }
                })
            );

            // Récupérer le CSS inline de la page
            const inlineCSS = Array.from(document.querySelectorAll('style'))
                .map(s => s.textContent ?? '')
                .join('\n');

            // Construire le fichier HTML autonome
            const html = `<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Portfolio — ${firstName}</title>
  <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=DM+Mono:wght@300;400;500&family=Fraunces:ital,opsz,wght@0,9..144,300;0,9..144,600;1,9..144,300&display=swap" />
...
${clone.outerHTML}
            ...
            // Télécharger directement le .html
            const blob = new Blob([html], { type: 'text/html;charset=utf-8' });
            const url  = URL.createObjectURL(blob);
            const a    = document.createElement('a');
            a.href     = url;
            a.download = `portfolio_${firstName}.html`
                .toLowerCase()
                .replace(/\s+/g, '_')
                .replace(/[^a-z0-9_.]/g, '');
    ...
    return { downloadSite, isDownloading };
};
...
const PortfolioView = ({ portfolio, collab }: { portfolio: any; collab: any }) => {
    ...
    const availLabel = availStatus === 'available' ? 'Disponible' : availStatus === 'soon' ? 'Bientôt disponible' : 'Non disponible';
    const availDot = availStatus === 'available' ? 'pf-dot-green' : availStatus === 'soon' ? 'pf-dot-orange' : 'pf-dot-muted';

   const { downloadSite, isDownloading } = usePortfolioDownload();

const handleDownload = () => {
    downloadSite(user?.firstName ?? 'portfolio', user?.lastName ?? '');
};
    ...
    const hasAnything = visibleSkills.length || visibleExperiences.length || visibleProjects.length || visibleCerts.length || visibleEducations.length;
        ...
        <div className="pf-root">
            ...
            <nav className="pf-nav">
                ...
                <ul className="pf-nav-links">
                    ...
                    {collab.linkedinUrl && (
                        <li>
                            <a href={collab.linkedinUrl} target="_blank" rel="noreferrer">
                                LinkedIn
                            </a>
                        </li>
                    )}
    ...
    {/* Supprimer le pf-nav-badge ou le garder à côté */}
    <div className="pf-nav-badge">{portfolio.title}</div>
            ...
            <div className="pf-hero">
                ...
                <div className="pf-hero-left">
                    <div className="pf-hero-tag">{portfolio.targetClient ? `Pour ${portfolio.targetClient}` : 'Portfolio professionnel'}</div>
                    ...
                    {(collab.githubUrl || collab.linkedinUrl || collab.city || collab.phone || portfolio.publicSlug) && (
                        <div className="pf-social-links">
                            ...
                            {collab.city && <span className="pf-social-link">📍 {collab.city}</span>}
                            {collab.phone && (
                                <a href={`tel:${collab.phone}`} className="pf-social-link">
                                    📞 {collab.phone}
                                </a>
                            )}
                        </div>
                ...
                <div className="pf-hero-right">
                    ...
                    <div className="pf-avatar-wrapper">
                        <img
                            ...
                            onError={(e) => {
                                (e.target as HTMLImageElement).src = 'https://ui-avatars.com/api/?name=?&background=0a0a0f&color=f5f2eb&size=200';
                            }}
                        />
                    </div>
                    ...
                    <div className="pf-avail-card">
                        ...
                        <div className="pf-avail-sub">{portfolio.targetClient ? `Dossier : ${portfolio.targetClient}` : 'Ouvert aux opportunités'}</div>
                    </div>
...
const MyPortfolioPage = () => {
        ...
        return (
            <>
                ...
                <div className="pf-no-data">
                    ...
                    <p style={{ maxWidth: 400, fontSize: 13, marginTop: 8 }}>{isPublicMode ? "Ce portfolio n'existe pas ou n'est plus disponible." : ''}</p>
    ...
    const effectiveCollab = isPublicMode ? currentPortfolio?.collaborator : collab;
        ...
        <>
            ...
            {!isPublicMode && portfolios.length > 1 && (
                <div className="pf-switcher">
                    ...
                    <select value={selectedPortfolioId ?? ''} onChange={(e) => dispatch(SelectPortfolio(Number(e.target.value)))}>
                        {portfolios.map((p: any) => (
                            <option key={p.portfolioId} value={p.portfolioId}>
                                {p.title}
                                {p.targetClient ? ` — ${p.targetClient}` : ''}
                            </option>
                        ))}
                    </select>
                    <a href="/portfolio-builder" className="pf-switcher-edit">
                        ✏ Éditer
                    </a>
                </div>
            )}
