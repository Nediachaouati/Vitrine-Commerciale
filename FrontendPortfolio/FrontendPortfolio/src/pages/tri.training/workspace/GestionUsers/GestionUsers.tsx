...
const GestionUsers = () => {
    ...
    const isClientTab = location.pathname.includes('clients');

    const activeRole: RoleEnum = isManagerTab ? RoleEnum.MANAGER : isClientTab ? RoleEnum.CLIENT : RoleEnum.COLLABORATEUR;
    // Redux State
    ...
    const filteredUsers: KcUser[] = useMemo(() => listUsers.filter((u: KcUser) => u.roles?.includes(activeRole)), [listUsers, activeRole]);
    ...
    const clientCount = listUsers.filter((u: KcUser) => u.roles?.includes(RoleEnum.CLIENT)).length;
    // States
...
*/
    ...
    useEffect(() => {
        if (msg === AdminMessages.CREATE) {
            // Remplace alert() par une notification plus claire
            Swal.fire({
                icon: 'success',
                title: 'Utilisateur créé !',
                html: `
        <p>Le compte a été créé avec succès.</p>
        <p style="margin-top:10px; color:#4361ee;">
      ...
      `,
        ...
        if (error) {
            Swal.fire({
                icon: 'error',
                title: 'Erreur',
                text: typeof error === 'string' ? error : 'Une erreur est survenue',
            });
        }
    }, [msg, error, dispatch]);
        ...
        fields: [
            ...
            { name: 'password', label: 'Mot de passe', type: 'password', placeholder: 'Minimum 6 caractères...' },
            {
                ...
                rsOptions: [
                    ...
                    { value: RoleEnum.CLIENT, label: 'Client' },
                ],
                disabled: true,
            },
        ],
    ...
    const getKcRole = () => {
  if (isManagerTab) return 'vitrine-manager';
  if (isClientTab)  return 'vitrine-client';
  return 'vitrine-collaborator';
};
    ...
    const table = useMantineReactTable({
        ...
        mantineSearchTextInputProps: {
            placeholder: `Rechercher un ${isManagerTab ? 'manager' : isClientTab ? 'client' : 'collaborateur'}...`,
        },
    ...
    return (
        <>
                ...
                <div className="flex items-center sm:flex-row flex-col sm:justify-between justify-center">
                    <div className="w-full">
                        <div className="flex items-center space-x-4">
                            <div className="h-10 w-10 bg-danger rounded-full flex shrink-0 justify-center items-center text-white font-bold text-sm">GU</div>
                            <div>
                    ...
                    <div className="ltr:ml-auto rtl:mr-auto mt-3 sm:mt-0">
                        ...
                        >
                            ...
                            Ajouter {isManagerTab ? 'Manager' : isClientTab ? 'Client' : 'Collaborateur'}
                        </button>
                    </div>
                </div>
                ...
                <div className="flex border-b border-white-light dark:border-[#1b2e4b]">
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-2.5 text-sm font-medium border-b-2 transition-colors ${
                            !isManagerTab ? 'border-info text-info bg-info/5' : 'border-transparent text-gray-500 hover:text-info hover:bg-gray-50 dark:hover:bg-[#1a2035]'
                        }`}
                    >
                        Collaborateurs
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-2.5 text-sm font-medium border-b-2 transition-colors ${
                            isManagerTab ? 'border-primary text-primary bg-primary/5' : 'border-transparent text-gray-500 hover:text-primary hover:bg-gray-50 dark:hover:bg-[#1a2035]'
                        }`}
                    >
                        Managers
                    ...
                    <button
                        onClick={() => navigate('/gestionusers/clients')}
                        className={`flex items-center gap-2 px-6 py-2.5 text-sm font-medium border-b-2 transition-colors ${
                            isClientTab ? 'border-success text-success bg-success/5' : 'border-transparent text-gray-500 hover:text-success hover:bg-gray-50 dark:hover:bg-[#1a2035]'
                        }`}
                    >
                        Clients
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold ${isClientTab ? 'bg-success/20 text-success' : 'bg-gray-100 dark:bg-[#1b2e4b] text-gray-400'}`}>
                            {clientCount}
                        </span>
                    </button>
                </div>
            ...
            <PopFormDynamicV3
                ...
                title={methodForm === MethodFormEnum.ADD ? `Ajouter un ${isManagerTab ? 'Manager' : isClientTab ? 'Client' : 'Collaborateur'}` : "Supprimer l'utilisateur"}
                message={methodForm === MethodFormEnum.DEL ? 'Voulez-vous vraiment supprimer cet utilisateur ? Cette action est irréversible.' : 'Remplissez les informations ci-dessous.'}
